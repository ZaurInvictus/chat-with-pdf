...
export function UploadDocumentsForm() {
  ...
  const ingest = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUploadStatus(json.error || "Failed to upload PDF");
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={ingest} className="flex flex-col gap-4 w-full">
      ...
      {uploadStatus && (
        <p
          className={`text-sm ${uploadStatus.includes("success") ? "text-green-500" : "text-red-500"}`}
        >
          {uploadStatus}
        </p>
      )}
      <Button type="submit" disabled={isLoading}>
        ...
        <span className={isLoading ? "hidden" : ""}>Upload PDF</span>
      </Button>
    </form>
  );
}
